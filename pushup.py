...
from plot_performance_pushup import plotgraph


@torch.no_grad()
def run_pushup(poseweights='yolov7-w6-pose.pt', source='static/uploads/bicep.mp4', device='cpu', curltracker=True, drawskeleton=True, recommendation = False, parity=""):
    ...
    if ext in ["mp4", "webm", "avi"] or (ext not in ["mp4", "webm", "avi"] and ext.isnumeric()):
        ...
        out_video_name = "static\\uploads\\output_pushup_" + parity if path.isnumeric(
        ) else "static\\uploads\\output_pushup_" + parity
